...
var _sfc_main$16 = {
	...
	setup(__props) {
		...
		const submitBalance = () => {
			...
			form.post(url, {
				...
				onError: (errors) => {
					...
					if (errors?.phone) form.setError("phone", errors.phone);
					if (errors?.bank_id) form.setError("bank_id", errors.bank_id);
					if (errors?.destination_type) form.setError("destination_type", errors.destination_type);
		...
		const loadSbpBanks = async () => {
			try {
				const data = await (await fetch("/balance/sbp-banks")).json();
			...
			_push(ssrRenderComponent(AppLayout_default, _attrs, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						_push(`<div class="balance-page" data-v-f6249c3b${_scopeId}><div class="main" data-v-f6249c3b${_scopeId}><section class="top-row" data-v-f6249c3b${_scopeId}><div class="card" data-v-f6249c3b${_scopeId}><h2 class="funding-card-title" data-v-f6249c3b${_scopeId}>${ssrInterpolate(isWithdrawal.value ? "Снятие средств" : "Пополнение баланса")}</h2><div class="balance-row" data-v-f6249c3b${_scopeId}><span class="balance-row-label" data-v-f6249c3b${_scopeId}>Текущий баланс:</span><span class="balance-row-amount" data-v-f6249c3b${_scopeId}>${ssrInterpolate(Number(__props.balance).toLocaleString("ru-RU"))} ₽</span></div><div class="toggle-row" data-v-f6249c3b${_scopeId}><button type="button" class="${ssrRenderClass([{ active: !isWithdrawal.value }, "toggle-btn"])}" data-v-f6249c3b${_scopeId}> Пополнение </button><button type="button" class="${ssrRenderClass([{ active: isWithdrawal.value }, "toggle-btn"])}"${ssrIncludeBooleanAttr(Number(__props.balance) <= 0) ? " disabled" : ""} data-v-f6249c3b${_scopeId}> Снятие </button></div><label class="input-label" data-v-f6249c3b${_scopeId}>${ssrInterpolate(isWithdrawal.value ? "Сумма снятия:" : "Сумма пополнения:")}</label><div class="input-wrapper" data-v-f6249c3b${_scopeId}><input${ssrRenderAttr("value", formattedAmount.value)} type="text" inputmode="numeric" placeholder="Введите сумму" data-v-f6249c3b${_scopeId}><span class="currency-suffix" data-v-f6249c3b${_scopeId}>₽</span></div><div class="${ssrRenderClass([{ visible: unref(form).errors.amount || withdrawError.value || flashMessage.value }, "message-wrapper"])}" data-v-f6249c3b${_scopeId}>`);
						if (unref(form).errors.amount) _push(`<p class="error-text" data-v-f6249c3b${_scopeId}>${ssrInterpolate(unref(form).errors.amount)}</p>`);
						else _push(`<!---->`);
						if (withdrawError.value) _push(`<p class="error-text" data-v-f6249c3b${_scopeId}>${ssrInterpolate(withdrawError.value)}</p>`);
						else _push(`<!---->`);
						if (flashMessage.value) _push(`<p class="success-text" data-v-f6249c3b${_scopeId}>${ssrInterpolate(flashMessage.value)}</p>`);
						else _push(`<!---->`);
						_push(`</div><div class="quick-amounts" data-v-f6249c3b${_scopeId}><!--[-->`);
						ssrRenderList(isWithdrawal.value ? withdrawalAmounts : quickAmounts, (amount) => {
							_push(`<button type="button" class="quick-amount-btn" data-v-f6249c3b${_scopeId}>${ssrInterpolate(amount.toLocaleString("ru-RU"))} ₽ </button>`);
						});
						_push(`<!--]--></div>`);
						if (isWithdrawal.value) {
							_push(`<div class="payout-form" data-v-f6249c3b${_scopeId}><label class="input-label" data-v-f6249c3b${_scopeId}>Способ вывода:</label>`);
							if (__props.payoutMethods.length > 0) {
								_push(`<div class="saved-methods" data-v-f6249c3b${_scopeId}><!--[-->`);
								ssrRenderList(__props.payoutMethods, (m) => {
									_push(`<button type="button" class="${ssrRenderClass([{ active: selectedMethodId.value === m.id }, "method-btn"])}" data-v-f6249c3b${_scopeId}>${ssrInterpolate(m.type === "bank_card" ? "Карта" : "СБП")} ${ssrInterpolate(m.masked_number)}</button>`);
								});
								_push(`<!--]--><button type="button" class="${ssrRenderClass([{ active: !selectedMethodId.value }, "method-btn"])}" data-v-f6249c3b${_scopeId}> Новый способ </button></div>`);
							} else _push(`<!---->`);
							if (!selectedMethodId.value) {
								_push(`<div class="new-method" data-v-f6249c3b${_scopeId}><div class="toggle-row" data-v-f6249c3b${_scopeId}><button type="button" class="${ssrRenderClass([{ active: unref(form).destination_type === "bank_card" }, "toggle-btn"])}" data-v-f6249c3b${_scopeId}> Банковская карта </button><button type="button" class="${ssrRenderClass([{ active: unref(form).destination_type === "sbp" }, "toggle-btn"])}" data-v-f6249c3b${_scopeId}> СБП </button></div>`);
								if (unref(form).destination_type === "bank_card") {
									_push(`<div class="input-group" data-v-f6249c3b${_scopeId}><label class="input-label" data-v-f6249c3b${_scopeId}>Номер карты:</label><div class="input-wrapper" data-v-f6249c3b${_scopeId}><input${ssrRenderAttr("value", unref(form).card_number)} type="text" inputmode="numeric" placeholder="0000 0000 0000 0000" maxlength="19" data-v-f6249c3b${_scopeId}></div>`);
									if (unref(form).errors.card_number) _push(`<p class="error-text" data-v-f6249c3b${_scopeId}>${ssrInterpolate(unref(form).errors.card_number)}</p>`);
									else _push(`<!---->`);
									_push(`</div>`);
								} else _push(`<!---->`);
								if (unref(form).destination_type === "sbp") {
									_push(`<div class="input-group" data-v-f6249c3b${_scopeId}><label class="input-label" data-v-f6249c3b${_scopeId}>Телефон:</label><div class="input-wrapper" data-v-f6249c3b${_scopeId}><input${ssrRenderAttr("value", unref(form).phone)} type="text" inputmode="tel" placeholder="79000000000" maxlength="11" data-v-f6249c3b${_scopeId}></div>`);
									if (unref(form).errors.phone) _push(`<p class="error-text" data-v-f6249c3b${_scopeId}>${ssrInterpolate(unref(form).errors.phone)}</p>`);
									else _push(`<!---->`);
									_push(`<label class="input-label" data-v-f6249c3b${_scopeId}>Банк:</label><div class="input-wrapper" data-v-f6249c3b${_scopeId}><select data-v-f6249c3b${_scopeId}><option value="" data-v-f6249c3b${ssrIncludeBooleanAttr(Array.isArray(unref(form).bank_id) ? ssrLooseContain(unref(form).bank_id, "") : ssrLooseEqual(unref(form).bank_id, "")) ? " selected" : ""}${_scopeId}>— Выберите банк —</option><!--[-->`);
									ssrRenderList(sbpBanks.value, (bank) => {
										_push(`<option${ssrRenderAttr("value", bank.bank_id)} data-v-f6249c3b${ssrIncludeBooleanAttr(Array.isArray(unref(form).bank_id) ? ssrLooseContain(unref(form).bank_id, bank.bank_id) : ssrLooseEqual(unref(form).bank_id, bank.bank_id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(bank.name)}</option>`);
									});
									_push(`<!--]--></select></div>`);
									if (unref(form).errors.bank_id) _push(`<p class="error-text" data-v-f6249c3b${_scopeId}>${ssrInterpolate(unref(form).errors.bank_id)}</p>`);
									else _push(`<!---->`);
									_push(`</div>`);
								} else _push(`<!---->`);
								_push(`<label class="save-method-label" data-v-f6249c3b${_scopeId}><input${ssrIncludeBooleanAttr(Array.isArray(unref(form).save_method) ? ssrLooseContain(unref(form).save_method, null) : unref(form).save_method) ? " checked" : ""} type="checkbox" data-v-f6249c3b${_scopeId}> Сохранить реквизиты для следующих выводов </label></div>`);
							} else _push(`<div class="saved-method-info" data-v-f6249c3b${_scopeId}><p class="method-info" data-v-f6249c3b${_scopeId}>Вывод на сохранённый реквизит</p></div>`);
							_push(`</div>`);
						} else _push(`<!---->`);
						_push(`<button class="btn-cta"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""} data-v-f6249c3b${_scopeId}>${ssrInterpolate(unref(form).processing ? isWithdrawal.value ? "Снятие..." : "Пополнение..." : isWithdrawal.value ? "Вывести средства" : "Пополнить баланс")}</button></div><div class="card chart-card" data-v-f6249c3b${_scopeId}><div class="chart-header" data-v-f6249c3b${_scopeId}><h2 class="chart-title" data-v-f6249c3b${_scopeId}>Аналитика</h2><div class="toggle-row" data-v-f6249c3b${_scopeId}><button type="button" class="${ssrRenderClass([{ active: chartView.value === "received" }, "toggle-btn"])}" data-v-f6249c3b${_scopeId}> Получено </button><button type="button" class="${ssrRenderClass([{ active: chartView.value === "spent" }, "toggle-btn"])}" data-v-f6249c3b${_scopeId}> Выведено </button></div></div><div class="chart-wrapper" data-v-f6249c3b${_scopeId}><div class="chart-container" data-v-f6249c3b${_scopeId}><div class="y-axis" data-v-f6249c3b${_scopeId}><!--[-->`);
						ssrRenderList(yAxisValues.value, (val) => {
							_push(`<span data-v-f6249c3b${_scopeId}>${ssrInterpolate(val.toLocaleString("ru-RU"))}</span>`);
						});
						_push(`<!--]--></div><div class="grid-lines" data-v-f6249c3b${_scopeId}><!--[-->`);
						ssrRenderList(5, (i) => {
							_push(`<div class="grid-line" style="${ssrRenderStyle({ top: (i - 1) * 20 + "%" })}" data-v-f6249c3b${_scopeId}></div>`);
						});
						_push(`<!--]--></div><div class="chart-bars" data-v-f6249c3b${_scopeId}><!--[-->`);
						ssrRenderList(chartData.value, (val, i) => {
							_push(`<div class="bar-group" data-v-f6249c3b${_scopeId}><div class="${ssrRenderClass([chartView.value, "bar"])}" style="${ssrRenderStyle({ height: val / chartMax.value * 100 + "%" })}" data-v-f6249c3b${_scopeId}></div></div>`);
						});
						_push(`<!--]--></div></div><div class="x-axis" data-v-f6249c3b${_scopeId}><!--[-->`);
						ssrRenderList(months, (m, i) => {
							_push(`<span data-v-f6249c3b${_scopeId}>${ssrInterpolate(m)}</span>`);
						});
						_push(`<!--]--></div><div class="${ssrRenderClass([{ visible: tooltip.value.visible }, "chart-tooltip"])}" style="${ssrRenderStyle({
							left: tooltip.value.x + "px",
							top: tooltip.value.y + "px"
						})}" data-v-f6249c3b${_scopeId}>${tooltip.value.text ?? ""}</div></div></div></section>`);
						if (__props.pendingTransactions && __props.pendingTransactions.length > 0) {
							_push(`<div class="card pending-section" data-v-f6249c3b${_scopeId}><h2 class="funding-card-title" data-v-f6249c3b${_scopeId}>Ожидающие зачисления</h2><div class="tx-list" data-v-f6249c3b${_scopeId}><!--[-->`);
							ssrRenderList(__props.pendingTransactions, (tx) => {
								_push(`<div class="tx-item pending" data-v-f6249c3b${_scopeId}><div class="tx-top" data-v-f6249c3b${_scopeId}><span class="tx-amount positive" data-v-f6249c3b${_scopeId}>+${ssrInterpolate(Number(tx.amount).toFixed(2))} ₽</span><span class="tx-status pending" data-v-f6249c3b${_scopeId}>Ожидание</span></div><div class="tx-desc" data-v-f6249c3b${_scopeId}>${ssrInterpolate(tx.type === "payment" ? "Оплата за работу" : tx.description)} `);
								if (tx.from_user) _push(`<span data-v-f6249c3b${_scopeId}> — ${ssrInterpolate(tx.from_user.name)}</span>`);
								else _push(`<!---->`);
								_push(`</div><div class="tx-date" data-v-f6249c3b${_scopeId}>Осталось: ${ssrInterpolate(getDaysRemaining(tx))} дн.</div></div>`);
							});
							_push(`<!--]--></div></div>`);
						} else _push(`<!---->`);
						_push(`<section class="history-section card" data-v-f6249c3b${_scopeId}><div class="history-header" data-v-f6249c3b${_scopeId}><h2 class="history-title" data-v-f6249c3b${_scopeId}>История транзакций</h2></div><div class="history-tabs" data-v-f6249c3b${_scopeId}><button class="${ssrRenderClass([{ active: historyTab.value === "received" }, "history-tab"])}" data-v-f6249c3b${_scopeId}> Получено </button><button class="${ssrRenderClass([{ active: historyTab.value === "sent" }, "history-tab"])}" data-v-f6249c3b${_scopeId}> Отправлено </button><button class="${ssrRenderClass([{ active: historyTab.value === "withdrawn" }, "history-tab"])}" data-v-f6249c3b${_scopeId}> Вывод </button></div><div class="history-columns" data-v-f6249c3b${_scopeId}><div class="${ssrRenderClass([{ active: historyTab.value === "received" }, "history-col"])}" data-v-f6249c3b${_scopeId}><div class="history-column-title" data-v-f6249c3b${_scopeId}><span class="dot dot-received" data-v-f6249c3b${_scopeId}></span> Получено </div><div class="tx-list" data-v-f6249c3b${_scopeId}><!--[-->`);
						ssrRenderList(receivedTransactions.value, (tx) => {
							_push(`<div class="tx-item" data-v-f6249c3b${_scopeId}><div class="tx-top" data-v-f6249c3b${_scopeId}><span class="tx-amount positive" data-v-f6249c3b${_scopeId}>+${ssrInterpolate(Number(tx.amount).toFixed(2))} ₽</span><span class="${ssrRenderClass(["tx-status", tx.status])}" data-v-f6249c3b${_scopeId}>${ssrInterpolate(tx.status === "completed" ? "Завершён" : "Ожидание")}</span></div><div class="tx-desc" data-v-f6249c3b${_scopeId}>${ssrInterpolate(getTransactionType(tx))}</div><div class="tx-date" data-v-f6249c3b${_scopeId}>${ssrInterpolate(formatDate(tx.created_at))}</div></div>`);
						});
						_push(`<!--]-->`);
						if (receivedTransactions.value.length === 0) _push(`<div class="tx-item empty" data-v-f6249c3b${_scopeId}>Нет транзакций</div>`);
						else _push(`<!---->`);
						_push(`</div></div><div class="${ssrRenderClass([{ active: historyTab.value === "sent" }, "history-col"])}" data-v-f6249c3b${_scopeId}><div class="history-column-title" data-v-f6249c3b${_scopeId}><span class="dot dot-sent" data-v-f6249c3b${_scopeId}></span> Отправлено </div><div class="tx-list" data-v-f6249c3b${_scopeId}><!--[-->`);
						ssrRenderList(sentTransactions.value, (tx) => {
							_push(`<div class="tx-item" data-v-f6249c3b${_scopeId}><div class="tx-top" data-v-f6249c3b${_scopeId}><span class="tx-amount negative" data-v-f6249c3b${_scopeId}>−${ssrInterpolate(Number(tx.amount).toFixed(2))} ₽</span><span class="${ssrRenderClass(["tx-status", tx.status])}" data-v-f6249c3b${_scopeId}>${ssrInterpolate(tx.status === "completed" ? "Завершён" : "Ожидание")}</span></div><div class="tx-desc" data-v-f6249c3b${_scopeId}>${ssrInterpolate(getTransactionType(tx))}</div><div class="tx-date" data-v-f6249c3b${_scopeId}>${ssrInterpolate(formatDate(tx.created_at))}</div></div>`);
						});
						_push(`<!--]-->`);
						if (sentTransactions.value.length === 0) _push(`<div class="tx-item empty" data-v-f6249c3b${_scopeId}>Нет транзакций</div>`);
						else _push(`<!---->`);
						_push(`</div></div><div class="${ssrRenderClass([{ active: historyTab.value === "withdrawn" }, "history-col"])}" data-v-f6249c3b${_scopeId}><div class="history-column-title" data-v-f6249c3b${_scopeId}><span class="dot dot-withdrawn" data-v-f6249c3b${_scopeId}></span> Вывод средств </div><div class="tx-list" data-v-f6249c3b${_scopeId}><!--[-->`);
						ssrRenderList(withdrawnTransactions.value, (tx) => {
							_push(`<div class="tx-item" data-v-f6249c3b${_scopeId}><div class="tx-top" data-v-f6249c3b${_scopeId}><span class="tx-amount withdrawn" data-v-f6249c3b${_scopeId}>−${ssrInterpolate(Number(tx.amount).toFixed(2))} ₽</span><span class="${ssrRenderClass(["tx-status", tx.status])}" data-v-f6249c3b${_scopeId}>${ssrInterpolate(tx.status === "completed" ? "Завершён" : "Ожидание")}</span></div><div class="tx-desc" data-v-f6249c3b${_scopeId}>${ssrInterpolate(getTransactionType(tx))}</div><div class="tx-date" data-v-f6249c3b${_scopeId}>${ssrInterpolate(formatDate(tx.created_at))}</div></div>`);
						});
						_push(`<!--]-->`);
						if (withdrawnTransactions.value.length === 0) _push(`<div class="tx-item empty" data-v-f6249c3b${_scopeId}>Нет транзакций</div>`);
						else _push(`<!---->`);
						_push(`</div></div></div>`);
						if (__props.transactions && __props.transactions.last_page > 1) {
							_push(`<div class="pagination" data-v-f6249c3b${_scopeId}>`);
							if (__props.transactions.current_page > 1) _push(`<button class="pagination-btn" data-v-f6249c3b${_scopeId}> ← Назад </button>`);
							else _push(`<!---->`);
							_push(`<span class="pagination-info" data-v-f6249c3b${_scopeId}>Страница ${ssrInterpolate(__props.transactions.current_page)} из ${ssrInterpolate(__props.transactions.last_page)}</span>`);
							if (__props.transactions.current_page < __props.transactions.last_page) _push(`<button class="pagination-btn" data-v-f6249c3b${_scopeId}> Вперёд → </button>`);
							else _push(`<!---->`);
							_push(`</div>`);
						} else _push(`<!---->`);
						...
						createVNode("section", { class: "top-row" }, [createVNode("div", { class: "card" }, [
							...
							isWithdrawal.value ? (openBlock(), createBlock("div", {
								...
								__props.payoutMethods.length > 0 ? (openBlock(), createBlock("div", {
									...
									}, toDisplayString(m.type === "bank_card" ? "Карта" : "СБП") + " " + toDisplayString(m.masked_number), 11, ["onClick"]);
								...
								!selectedMethodId.value ? (openBlock(), createBlock("div", {
									...
									createVNode("div", { class: "toggle-row" }, [createVNode("button", {
										type: "button",
										class: ["toggle-btn", { active: unref(form).destination_type === "bank_card" }],
										onClick: ($event) => unref(form).destination_type = "bank_card"
									}, " Банковская карта ", 10, ["onClick"]), createVNode("button", {
										type: "button",
										class: ["toggle-btn", { active: unref(form).destination_type === "sbp" }],
										onClick: ($event) => {
											unref(form).destination_type = "sbp";
											loadSbpBanks();
										}
									}, " СБП ", 10, ["onClick"])]),
									unref(form).destination_type === "bank_card" ? (openBlock(), createBlock("div", {
										key: 0,
										class: "input-group"
									}, [
									...
									unref(form).destination_type === "sbp" ? (openBlock(), createBlock("div", {
										key: 1,
										class: "input-group"
									}, [
										createVNode("label", { class: "input-label" }, "Телефон:"),
										createVNode("div", { class: "input-wrapper" }, [withDirectives(createVNode("input", {
										...
										createVNode("label", { class: "input-label" }, "Банк:"),
										createVNode("div", { class: "input-wrapper" }, [withDirectives(createVNode("select", { "onUpdate:modelValue": ($event) => unref(form).bank_id = $event }, [createVNode("option", { value: "" }, "— Выберите банк —"), (openBlock(true), createBlock(Fragment, null, renderList(sbpBanks.value, (bank) => {
											return openBlock(), createBlock("option", {
												key: bank.bank_id,
												value: bank.bank_id
											}, toDisplayString(bank.name), 9, ["value"]);
										}), 128))], 8, ["onUpdate:modelValue"]), [[vModelSelect, unref(form).bank_id]])]),
										unref(form).errors.bank_id ? (openBlock(), createBlock("p", {
											key: 1,
											class: "error-text"
										}, toDisplayString(unref(form).errors.bank_id), 1)) : createCommentVNode("", true)
									])) : createCommentVNode("", true),
									createVNode("label", { class: "save-method-label" }, [withDirectives(createVNode("input", {
										...
										type: "checkbox"
									}, null, 8, ["onUpdate:modelValue"]), [[vModelCheckbox, unref(form).save_method]]), createTextVNode(" Сохранить реквизиты для следующих выводов ")])
								])) : (openBlock(), createBlock("div", {
									key: 2,
									class: "saved-method-info"
								}, [createVNode("p", { class: "method-info" }, "Вывод на сохранённый реквизит")]))
							])) : createCommentVNode("", true),
							createVNode("button", {
								class: "btn-cta",
...
var Index_default$2 = /* @__PURE__ */ _plugin_vue_export_helper_default(_sfc_main$16, [["__scopeId", "data-v-f6249c3b"]]);
//#endregion
			...
			_push(ssrRenderComponent(AppLayout_default, _attrs, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						_push(`<div class="chat-container" data-v-7b99f7f9${_scopeId}><div class="chat-list" data-v-7b99f7f9${_scopeId}>`);
						if (!isSearching.value) _push(`<div class="chat-list-header" data-v-7b99f7f9${_scopeId}><h2 data-v-7b99f7f9${_scopeId}>Чаты</h2><button type="button" class="search-btn" data-v-7b99f7f9${_scopeId}><img src="/images/search.svg" alt="Поиск" data-v-7b99f7f9${_scopeId}></button></div>`);
						else _push(`<div class="search-header" data-v-7b99f7f9${_scopeId}><input${ssrRenderAttr("value", searchQuery.value)} type="text"${ssrRenderAttr("placeholder", isGlobalSearch.value ? "Поиск чатов и сообщений..." : "Поиск в чате...")} class="search-input" data-v-7b99f7f9${_scopeId}><button type="button" class="search-close" data-v-7b99f7f9${_scopeId}><img src="/images/close.svg" alt="Закрыть" data-v-7b99f7f9${_scopeId}></button></div>`);
						if (!isSearching.value) {
							...
							ssrRenderList(__props.chats, (chat) => {
								...
								}, "chat-item"])}" tabindex="0" data-v-7b99f7f9${_scopeId}>`);
								if (chat.other_user) {
									_push(`<div class="chat-user-info" data-v-7b99f7f9${_scopeId}><div class="avatar-wrapper" data-v-7b99f7f9${_scopeId}><img${ssrRenderAttr("src", chat.other_user.avatar_url)} class="chat-avatar" data-v-7b99f7f9${_scopeId}>`);
									if (isUserOnline(chat.other_user.id)) _push(`<span class="online-indicator" data-v-7b99f7f9${_scopeId}></span>`);
									else _push(`<!---->`);
									_push(`</div><div data-v-7b99f7f9${_scopeId}><h3 data-v-7b99f7f9${_scopeId}>${ssrInterpolate(chat.other_user.name)}</h3>`);
									if (chat.latest_message) _push(`<p class="${ssrRenderClass([{ unread: chat.unread_count > 0 }, "chat-preview"])}" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(getChatPreview(chat.latest_message))}</p>`);
									else _push(`<!---->`);
									_push(`</div></div>`);
								} else _push(`<!---->`);
								_push(`<div class="chat-meta" data-v-7b99f7f9${_scopeId}>`);
								if (chat.unread_count > 0) _push(`<span class="unread-badge" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(chat.unread_count > 99 ? "99+" : chat.unread_count)}</span>`);
								else _push(`<!---->`);
								_push(`</div></div>`);
							});
							_push(`<!--]-->`);
						...
						if (isSearching.value) {
							_push(`<div class="search-results" data-v-7b99f7f9${_scopeId}>`);
							if (searchResults.value.length === 0) _push(`<div class="search-no-results" data-v-7b99f7f9${_scopeId}> Ничего не найдено </div>`);
							else _push(`<!---->`);
							_push(`<!--[-->`);
							ssrRenderList(searchResults.value, (result) => {
								_push(`<div class="search-result-item" data-v-7b99f7f9${_scopeId}><img${ssrRenderAttr("src", result.user.avatar_url)} class="chat-avatar" data-v-7b99f7f9${_scopeId}><div class="search-result-content" data-v-7b99f7f9${_scopeId}><div class="search-result-name" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(result.user.name)}</div>`);
								if (result.type === "chat") _push(`<div class="search-result-type" data-v-7b99f7f9${_scopeId}>Чат</div>`);
								else _push(`<div class="search-result-text" data-v-7b99f7f9${_scopeId}>${highlight(result.content) ?? ""}</div>`);
								_push(`</div></div>`);
							});
							_push(`<!--]--></div>`);
						} else _push(`<!---->`);
						...
						}, "chat-area"])}" style="${ssrRenderStyle(slideOffset.value > 0 ? { transform: `translateX(${slideOffset.value}px)` } : {})}" data-v-7b99f7f9${_scopeId}>`);
						if (__props.activeChat) {
							_push(`<!--[--><div class="chat-header" data-v-7b99f7f9${_scopeId}><button type="button" class="back" data-v-7b99f7f9${_scopeId}><img src="/images/arrow-left.svg" alt="назад" data-v-7b99f7f9${_scopeId}></button><div class="chat-header-mid" data-v-7b99f7f9${_scopeId}>`);
							if (otherUsers.value.length > 0) _push(ssrRenderComponent(unref(Link), {
								...
								default: withCtx((_, _push, _parent, _scopeId) => {
									if (_push) {
										_push(`<div class="avatar-wrapper" data-v-7b99f7f9${_scopeId}><img${ssrRenderAttr("src", otherUsers.value[0].avatar_url)} class="chat-avatar" data-v-7b99f7f9${_scopeId}>`);
										if (isUserOnline(otherUsers.value[0].id)) _push(`<span class="online-indicator" data-v-7b99f7f9${_scopeId}></span>`);
										else _push(`<span class="offline-indicator" data-v-7b99f7f9${_scopeId}></span>`);
										_push(`</div><h2 data-v-7b99f7f9${_scopeId}>${ssrInterpolate(otherUsers.value[0].name)}</h2>`);
							...
							if (vacancyPostId.value) _push(`<span class="vacancy-link" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(isVacancyAuthor.value ? "откликнулся на" : "автор вакансии")}</span>`);
							else _push(`<!---->`);
							_push(ssrRenderComponent(unref(Link), {
								...
								default: withCtx((_, _push, _parent, _scopeId) => {
									if (_push) _push(`<h2 data-v-7b99f7f9${_scopeId}>${ssrInterpolate(vacancyPosition.value)}</h2>`);
									else return [createVNode("h2", null, toDisplayString(vacancyPosition.value), 1)];
								}),
								_: 1
							}, _parent, _scopeId));
							_push(`</div><img class="chat-options" src="/images/dots.svg" alt="опции" data-v-7b99f7f9${_scopeId}></div>`);
							if (isSearching.value && !isGlobalSearch.value && __props.activeChat) _push(`<div class="search-navigation" data-v-7b99f7f9${_scopeId}><button type="button" class="search-nav-btn"${ssrIncludeBooleanAttr(currentMatchIndex.value <= 0) ? " disabled" : ""} data-v-7b99f7f9${_scopeId}><img src="/images/arrow-up.svg" alt="вверх" data-v-7b99f7f9${_scopeId}></button><span class="search-nav-counter" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(searchResults.value.length > 0 ? currentMatchIndex.value + 1 : 0)} / ${ssrInterpolate(searchResults.value.length)}</span><button type="button" class="search-nav-btn"${ssrIncludeBooleanAttr(currentMatchIndex.value >= searchResults.value.length - 1) ? " disabled" : ""} data-v-7b99f7f9${_scopeId}><img src="/images/arrow-up.svg" alt="вниз" style="${ssrRenderStyle({ "transform": "rotate(180deg)" })}" data-v-7b99f7f9${_scopeId}></button></div>`);
							else _push(`<!---->`);
							...
							})}" data-v-7b99f7f9${_scopeId}><div class="options-menu-item" data-v-7b99f7f9${_scopeId}>Файлы чата</div><div class="options-menu-item" data-v-7b99f7f9${_scopeId}>Поиск по чату</div><div class="options-menu-item delete" data-v-7b99f7f9${_scopeId}>Удалить чат</div></div>`);
							else _push(`<!---->`);
							if (showApplicationBlock.value) {
								_push(`<div class="${ssrRenderClass([{ closed: isApplicationBlockClosed.value }, "application-block"])}" data-v-7b99f7f9${_scopeId}><div class="application-toggle" data-v-7b99f7f9${_scopeId}><button type="button" class="application-toggle-btn" data-v-7b99f7f9${_scopeId}><img src="/images/arrow-up.svg" alt="Toggle" class="${ssrRenderClass([{ flipped: isApplicationBlockClosed.value }, "toggle-arrow"])}" data-v-7b99f7f9${_scopeId}></button></div><div class="application-card" style="${ssrRenderStyle(!isApplicationBlockClosed.value ? null : { display: "none" })}" data-v-7b99f7f9${_scopeId}><img${ssrRenderAttr("src", __props.activeChat.application.user.avatar_url)} class="application-avatar" data-v-7b99f7f9${_scopeId}><div class="name" data-v-7b99f7f9${_scopeId}><h3 data-v-7b99f7f9${_scopeId}>${ssrInterpolate(__props.activeChat.application.user.name)}</h3><span class="${ssrRenderClass([{ online: isUserOnline(otherUsers.value[0].id) }, "user-status"])}" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(isUserOnline(otherUsers.value[0].id) ? "онлайн" : "оффлайн")}</span></div><p class="account-age" data-v-7b99f7f9${_scopeId}>Аккаунт создан ${ssrInterpolate(formatAccountAge(__props.activeChat.application.user.created_at))}</p>`);
								if (__props.activeChat.application.user.rating) _push(`<div class="application-rating" data-v-7b99f7f9${_scopeId}><span data-v-7b99f7f9${_scopeId}>Рейтинг: ${ssrInterpolate(__props.activeChat.application.user.rating)}</span></div>`);
								else _push(`<!---->`);
								_push(`<div class="application-cover-letter" data-v-7b99f7f9${_scopeId}><h4 data-v-7b99f7f9${_scopeId}>Сопроводительное письмо:</h4><p data-v-7b99f7f9${_scopeId}>${ssrInterpolate(__props.activeChat.application.cover_letter)}</p></div>`);
								if (__props.activeChat.application.proposed_price) {
									_push(`<div class="application-price-row" data-v-7b99f7f9${_scopeId}><div class="application-price" data-v-7b99f7f9${_scopeId}><span class="label" data-v-7b99f7f9${_scopeId}>Предложенная цена:</span><span class="value" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(__props.activeChat.application.proposed_price)} ₽</span></div>`);
									if (isVacancyAuthor.value && __props.activeChat.application.status === "pending") _push(`<button type="button" class="price-edit-btn" data-v-7b99f7f9${_scopeId}><img src="/images/edit.svg" alt="Изменить цену" data-v-7b99f7f9${_scopeId}></button>`);
									else _push(`<!---->`);
									_push(`</div>`);
								} else _push(`<!---->`);
								if (__props.activeChat.application.status === "completed") _push(`<div class="application-status" data-v-7b99f7f9${_scopeId}><span class="status-badge completed" data-v-7b99f7f9${_scopeId}>✅ Сделка завершена</span></div>`);
								else _push(`<!---->`);
								if (__props.activeChat.application.status === "cancelled") _push(`<div class="application-status" data-v-7b99f7f9${_scopeId}><span class="status-badge cancelled" data-v-7b99f7f9${_scopeId}>❌ Сделка отменена</span></div>`);
								else _push(`<!---->`);
								if (__props.activeChat.application.status === "disputed" || __props.activeChat.application.dispute && __props.activeChat.application.dispute.status === "open") _push(`<div class="application-status" data-v-7b99f7f9${_scopeId}><span class="status-badge dispute" data-v-7b99f7f9${_scopeId}>⚠️ Открыт спор</span></div>`);
								else _push(`<!---->`);
								if (isVacancyAuthor.value) {
									_push(`<div class="application-actions" data-v-7b99f7f9${_scopeId}>`);
									if (__props.activeChat.application.status === "pending") _push(`<!--[--><form data-v-7b99f7f9${_scopeId}><button type="submit" class="accept-btn" data-v-7b99f7f9${_scopeId}>Принять отклик</button></form><form data-v-7b99f7f9${_scopeId}><button type="submit" class="reject-btn" data-v-7b99f7f9${_scopeId}>Отклонить</button></form><!--]-->`);
									else if (__props.activeChat.application.status === "in_progress") {
										_push(`<!--[-->`);
										if (__props.activeChat.application.executor_marked_completed_at) _push(`<form data-v-7b99f7f9${_scopeId}><button type="submit" class="accept-btn" data-v-7b99f7f9${_scopeId}>Подтвердить завершение</button></form>`);
										else _push(`<!---->`);
										if (!__props.activeChat.application.executor_marked_completed_at) _push(`<button type="button" class="withdraw-btn" data-v-7b99f7f9${_scopeId}> Отменить </button>`);
										else _push(`<!---->`);
										_push(`<!--]-->`);
									} else if (__props.activeChat.application.status === "accepted") _push(`<button type="button" class="withdraw-btn" data-v-7b99f7f9${_scopeId}> Отменить </button>`);
									else _push(`<!---->`);
									_push(`</div>`);
								} else _push(`<!---->`);
								if (!isVacancyAuthor.value) {
									_push(`<div class="application-actions" data-v-7b99f7f9${_scopeId}>`);
									if (__props.activeChat.application.status === "in_progress" && !__props.activeChat.application.executor_marked_completed_at) _push(`<!--[--><form data-v-7b99f7f9${_scopeId}><button type="submit" class="accept-btn" data-v-7b99f7f9${_scopeId}>Отметить как выполненное</button></form><button type="button" class="withdraw-btn" data-v-7b99f7f9${_scopeId}> Отменить </button><!--]-->`);
									else _push(`<!---->`);
									if (__props.activeChat.application.status === "in_progress" && __props.activeChat.application.executor_marked_completed_at) _push(`<!--[--><button type="button" class="withdraw-btn" data-v-7b99f7f9${_scopeId}> Отменить </button><button type="button" class="dispute-btn" data-v-7b99f7f9${_scopeId}> Открыть спор </button><!--]-->`);
									else _push(`<!---->`);
									if (__props.activeChat.application.status === "accepted") _push(`<button type="button" class="dispute-btn" data-v-7b99f7f9${_scopeId}> Открыть спор </button>`);
									else _push(`<!---->`);
									_push(`</div>`);
								} else _push(`<!---->`);
								_push(`</div></div>`);
							} else _push(`<!---->`);
							_push(`<div class="chat-messages" data-v-7b99f7f9${_scopeId}><div class="chat-messages-inner" data-v-7b99f7f9${_scopeId}><div${ssrRenderAttrs({
								name: "messages",
							...
							ssrRenderList(localMessages.value, (message, index) => {
								...
								}, "message-container"])}" data-v-7b99f7f9${_scopeId}><img${ssrRenderAttr("src", message.user.avatar_url)} class="chat-avatar" data-v-7b99f7f9${_scopeId}><div class="${ssrRenderClass([{
									"my-message": message.is_mine,
									"shared-post": isOnlyPostUrl(message.content)
								}, "message"])}" data-v-7b99f7f9${_scopeId}>`);
								if (message.image_url) _push(`<img${ssrRenderAttr("src", message.image_url)} alt="Изображение" class="message-image" data-v-7b99f7f9${_scopeId}>`);
								else _push(`<!---->`);
								if (message.video_url) _push(`<video${ssrRenderAttr("src", message.video_url)} class="message-video" controls data-v-7b99f7f9${_scopeId}></video>`);
								else _push(`<!---->`);
								_push(`<div class="message-content" data-v-7b99f7f9${_scopeId}>`);
								if (message.content && !isOnlyPostUrl(message.content)) _push(`<span data-v-7b99f7f9${_scopeId}>${renderContent(message.content) ?? ""}</span>`);
								else _push(`<!---->`);
								_push(`<!--[-->`);
								ssrRenderList(getPostPreviewsFromContent(message.content), (preview) => {
									_push(`<div class="${ssrRenderClass([{ "shared-post-card": isOnlyPostUrl(message.content) }, "post-preview-card"])}" data-v-7b99f7f9${_scopeId}>`);
									if (preview.image_url) _push(`<img${ssrRenderAttr("src", preview.image_url)} class="post-preview-img" alt="" data-v-7b99f7f9${_scopeId}>`);
									else _push(`<!---->`);
									_push(`<div class="post-preview-body" data-v-7b99f7f9${_scopeId}><div class="post-preview-title" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(preview.title)}</div>`);
									if (preview.description) _push(`<div class="post-preview-desc" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(preview.description.slice(0, 80))}${ssrInterpolate(preview.description.length > 80 ? "…" : "")}</div>`);
									else _push(`<!---->`);
									_push(`</div></div>`);
								});
								_push(`<!--]-->`);
								if (message.file_url) {
									_push(`<div class="file-attachment" data-v-7b99f7f9${_scopeId}><button type="button" class="file-download-circle"${ssrRenderAttr("title", isFileDownloaded(message) ? "Скачано" : "Скачать")} data-v-7b99f7f9${_scopeId}>`);
									if (!isFileDownloaded(message)) _push(`<span data-v-7b99f7f9${_scopeId}><img src="/images/download.svg" alt="Скачать" data-v-7b99f7f9${_scopeId}></span>`);
									else _push(`<span data-v-7b99f7f9${_scopeId}><img src="/images/document.svg" alt="Файл" data-v-7b99f7f9${_scopeId}></span>`);
									_push(`</button><div class="file-meta" data-v-7b99f7f9${_scopeId}><div class="file-name" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(message.file_name || "Файл")}</div>`);
									if (message.file_size) _push(`<div class="file-size" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(formatSize(message.file_size))}</div>`);
									else _push(`<!---->`);
									_push(`</div></div>`);
								} else _push(`<!---->`);
								if (message.is_price_proposal && message.price_proposal_status === "pending" && !message.is_mine) _push(`<div class="price-proposal-actions" data-v-7b99f7f9${_scopeId}><form class="price-action-form" data-v-7b99f7f9${_scopeId}><button type="submit" class="accept-price-btn" data-v-7b99f7f9${_scopeId}>Принять</button></form><button type="button" class="change-price-btn" data-v-7b99f7f9${_scopeId}>Изменить</button></div>`);
								else _push(`<!---->`);
								_push(`</div>`);
								if (message.reply_to) _push(`<div class="reply-quote" data-v-7b99f7f9${_scopeId}><div class="reply-quote-author" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(message.reply_to.user.name)}</div><div class="reply-quote-text" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(message.reply_to.content?.slice(0, 100))}</div></div>`);
								else _push(`<!---->`);
								_push(`<div class="message-time" data-v-7b99f7f9${_scopeId}><span data-v-7b99f7f9${_scopeId}>${ssrInterpolate(message.time)}</span>`);
								if (getMessageStatus(message)) {
									_push(`<span class="${ssrRenderClass([getMessageStatus(message), "message-status"])}" data-v-7b99f7f9${_scopeId}>`);
									if (getMessageStatus(message) === "sending") _push(`<img src="/images/loading.svg" alt="Отправляется" class="status-icon spinning" data-v-7b99f7f9${_scopeId}>`);
									else if (getMessageStatus(message) === "sent") _push(`<img src="/images/check-mark.svg" alt="Отправлено" class="status-icon" data-v-7b99f7f9${_scopeId}>`);
										...
										"width": "16px",
										"height": "16px"
									})}" data-v-7b99f7f9${_scopeId}>`);
									else if (getMessageStatus(message) === "failed") _push(`<img src="/images/exclamation-circle.svg" alt="Не отправлено" class="status-icon" data-v-7b99f7f9${_scopeId}>`);
									else _push(`<!---->`);
									_push(`</span>`);
								} else _push(`<!---->`);
								_push(`</div></div></div>`);
							...
							if (showSkeleton.value) {
								_push(`<div class="${ssrRenderClass([{ fading: isSkeletonFading.value }, "chat-skeleton"])}" data-v-7b99f7f9${_scopeId}><!--[-->`);
								ssrRenderList(skeletonItems.value, (item, index) => {
									_push(`<div class="${ssrRenderClass([{ right: item.side === "right" }, "skeleton-message"])}" data-v-7b99f7f9${_scopeId}><div class="skeleton-avatar" data-v-7b99f7f9${_scopeId}></div><div class="skeleton-bubble" data-v-7b99f7f9${_scopeId}><div class="skeleton-line" style="${ssrRenderStyle({ width: item.lines[0] })}" data-v-7b99f7f9${_scopeId}></div><div class="skeleton-line short" style="${ssrRenderStyle({ width: item.lines[1] })}" data-v-7b99f7f9${_scopeId}></div>`);
									if (item.hasThirdLine) _push(`<div class="skeleton-line tiny" style="${ssrRenderStyle({ width: item.lines[2] })}" data-v-7b99f7f9${_scopeId}></div>`);
									else _push(`<!---->`);
									_push(`</div></div>`);
								});
								_push(`<!--]--></div>`);
							} else _push(`<!---->`);
							_push(`</div></div><form class="message-form" enctype="multipart/form-data" data-v-7b99f7f9${_scopeId}>`);
							if (editingMessage.value) _push(`<div class="editing-indicator" data-v-7b99f7f9${_scopeId}><span data-v-7b99f7f9${_scopeId}>Редактирование сообщения</span><button type="button" class="cancel-edit-btn" data-v-7b99f7f9${_scopeId}>✕</button></div>`);
							else _push(`<!---->`);
							if (replyingTo.value) _push(`<div class="reply-indicator" data-v-7b99f7f9${_scopeId}><div class="reply-preview" data-v-7b99f7f9${_scopeId}><span class="reply-name" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(replyingTo.value.user.name)}</span><span class="reply-text" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(replyingTo.value.content?.slice(0, 100))}</span></div><button type="button" class="cancel-reply-btn" data-v-7b99f7f9${_scopeId}>✕</button></div>`);
							else _push(`<!---->`);
							if (photoPreviewUrl.value || videoPreviewUrl.value || documentPreviewName.value) {
								_push(`<div class="image-preview-container" style="${ssrRenderStyle({ "display": "flex" })}" data-v-7b99f7f9${_scopeId}>`);
								if (photoPreviewUrl.value) _push(`<img${ssrRenderAttr("src", photoPreviewUrl.value)} alt="Превью" class="image-preview" data-v-7b99f7f9${_scopeId}>`);
								else _push(`<!---->`);
								if (videoPreviewUrl.value) _push(`<video${ssrRenderAttr("src", videoPreviewUrl.value)} class="video-preview" controls data-v-7b99f7f9${_scopeId}></video>`);
								else _push(`<!---->`);
								if (documentPreviewName.value) _push(`<div class="file-preview" data-v-7b99f7f9${_scopeId}> 📎 ${ssrInterpolate(documentPreviewName.value)}</div>`);
								else _push(`<!---->`);
								_push(`<div class="preview-actions" data-v-7b99f7f9${_scopeId}><button type="button" class="select-other-btn" data-v-7b99f7f9${_scopeId}> Выбрать другое </button>`);
								if (!editingMessage.value) _push(`<button type="button" class="cancel-preview-btn" data-v-7b99f7f9${_scopeId}> Отмена </button>`);
								else _push(`<!---->`);
								_push(`</div></div>`);
							} else _push(`<!---->`);
							_push(`<div class="message-input-container" data-v-7b99f7f9${_scopeId}>`);
							if (!editingMessage.value) _push(`<div class="add" data-v-7b99f7f9${_scopeId}><img src="/images/clip.svg" alt="Добавить вложение" data-v-7b99f7f9${_scopeId}><div class="add-select" data-v-7b99f7f9${_scopeId}><label data-v-7b99f7f9${_scopeId}> Фото <input type="file" name="photo" accept="image/*" data-v-7b99f7f9${_scopeId}></label><label data-v-7b99f7f9${_scopeId}> Видео <input type="file" name="video" accept="video/*" data-v-7b99f7f9${_scopeId}></label><label data-v-7b99f7f9${_scopeId}> Документ <input type="file" name="document" data-v-7b99f7f9${_scopeId}></label></div></div>`);
							else _push(`<!---->`);
							_push(`<textarea${ssrRenderAttr("placeholder", editingMessage.value ? "Редактируйте сообщение..." : "Введите сообщение...")} data-v-7b99f7f9${_scopeId}>${ssrInterpolate(unref(form).content)}</textarea><button type="submit"${ssrIncludeBooleanAttr(unref(form).processing || !canSend.value) ? " disabled" : ""} data-v-7b99f7f9${_scopeId}>${ssrInterpolate(editingMessage.value ? "Сохранить" : "Отправить")}</button></div></form><!--]-->`);
						} else _push(`<div class="chat-placeholder" data-v-7b99f7f9${_scopeId}><p data-v-7b99f7f9${_scopeId}>Выберите чат для начала общения</p></div>`);
						_push(`</div>`);
						if (showChatFiles.value) {
							_push(`<div class="chat-files-panel" data-v-7b99f7f9${_scopeId}><div class="chat-files-header" data-v-7b99f7f9${_scopeId}><h3 data-v-7b99f7f9${_scopeId}>Файлы чата</h3><button type="button" class="chat-files-close" data-v-7b99f7f9${_scopeId}><img src="/images/close.svg" alt="Закрыть" data-v-7b99f7f9${_scopeId}></button></div><div class="chat-files-tabs" data-v-7b99f7f9${_scopeId}><button type="button" class="${ssrRenderClass([{ active: chatFilesTab.value === "media" }, "chat-files-tab"])}" data-v-7b99f7f9${_scopeId}> Медиа </button><button type="button" class="${ssrRenderClass([{ active: chatFilesTab.value === "files" }, "chat-files-tab"])}" data-v-7b99f7f9${_scopeId}> Файлы </button><button type="button" class="${ssrRenderClass([{ active: chatFilesTab.value === "links" }, "chat-files-tab"])}" data-v-7b99f7f9${_scopeId}> Ссылки </button></div><div class="chat-files-content" data-v-7b99f7f9${_scopeId}>`);
							if (chatFilesTab.value === "media") {
								_push(`<div class="chat-files-media" data-v-7b99f7f9${_scopeId}>`);
								if (chatMediaFiles.value.length === 0) _push(`<div class="chat-files-empty" data-v-7b99f7f9${_scopeId}> Нет медиафайлов </div>`);
								else {
									_push(`<div class="chat-files-grid" data-v-7b99f7f9${_scopeId}><!--[-->`);
									ssrRenderList(chatMediaFiles.value, (media) => {
										_push(`<div class="chat-files-media-item" data-v-7b99f7f9${_scopeId}>`);
										if (media.type === "image") _push(`<img${ssrRenderAttr("src", media.url)} alt="Медиа" data-v-7b99f7f9${_scopeId}>`);
										else _push(`<video${ssrRenderAttr("src", media.url)} data-v-7b99f7f9${_scopeId}></video>`);
										if (media.type === "video") _push(`<div class="play-icon" data-v-7b99f7f9${_scopeId}>▶</div>`);
										else _push(`<!---->`);
										_push(`</div>`);
									});
									_push(`<!--]--></div>`);
							...
							if (chatFilesTab.value === "files") {
								_push(`<div class="chat-files-list" data-v-7b99f7f9${_scopeId}>`);
								if (chatDocFiles.value.length === 0) _push(`<div class="chat-files-empty" data-v-7b99f7f9${_scopeId}> Нет файлов </div>`);
								else {
									_push(`<div data-v-7b99f7f9${_scopeId}><!--[-->`);
									ssrRenderList(chatDocFiles.value, (file) => {
										_push(`<div class="chat-files-item" data-v-7b99f7f9${_scopeId}><button type="button" class="file-download-circle" data-v-7b99f7f9${_scopeId}><img src="/images/download.svg" alt="Скачать" data-v-7b99f7f9${_scopeId}></button><div class="file-info" data-v-7b99f7f9${_scopeId}><div class="file-name" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(file.file_name)}</div><div class="file-size" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(formatSize(file.file_size))}</div></div></div>`);
									});
									_push(`<!--]--></div>`);
								}
								_push(`</div>`);
							...
							if (chatFilesTab.value === "links") {
								_push(`<div class="chat-files-list" data-v-7b99f7f9${_scopeId}>`);
								if (chatLinks.value.length === 0) _push(`<div class="chat-files-empty" data-v-7b99f7f9${_scopeId}> Нет ссылок </div>`);
								else {
									_push(`<div data-v-7b99f7f9${_scopeId}><!--[-->`);
									ssrRenderList(chatLinks.value, (link) => {
										_push(`<div class="chat-files-link" data-v-7b99f7f9${_scopeId}><span class="link-text" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(link.content)}</span><span class="link-time" data-v-7b99f7f9${_scopeId}>${ssrInterpolate(link.time)}</span></div>`);
									});
									_push(`<!--]--></div>`);
								}
								_push(`</div>`);
						...
						_push(`</div>`);
						if (modalOpen.value && modalImage.value) _push(`<div class="modal-overlay" style="${ssrRenderStyle({ "display": "flex" })}" data-v-7b99f7f9${_scopeId}><div class="modal-content" data-v-7b99f7f9${_scopeId}><button class="modal-close" type="button" data-v-7b99f7f9${_scopeId}> × </button><img${ssrRenderAttr("src", modalImage.value)} alt="Предпросмотр" data-v-7b99f7f9${_scopeId}></div></div>`);
						else _push(`<!---->`);
						if (modalOpen.value && modalVideo.value) _push(`<div class="modal-overlay" style="${ssrRenderStyle({ "display": "flex" })}" data-v-7b99f7f9${_scopeId}><div class="modal-content modal-video-content" data-v-7b99f7f9${_scopeId}><button class="modal-close" type="button" data-v-7b99f7f9${_scopeId}> × </button><video${ssrRenderAttr("src", modalVideo.value)} controls autoplay class="modal-video" data-v-7b99f7f9${_scopeId}></video></div></div>`);
						else _push(`<!---->`);
						if (contextMenu.value.show) {
							...
							if (!contextMenu.value.message?.is_mine) _push(`<div class="context-menu-item" data-v-7b99f7f9${_scopeId}> Ответить </div>`);
							else _push(`<!---->`);
							if (contextMenu.value.message?.is_mine) _push(`<div class="context-menu-item" data-v-7b99f7f9${_scopeId}> Редактировать </div>`);
							else _push(`<!---->`);
							if (contextMenu.value.message?.is_mine) _push(`<div class="context-menu-item" data-v-7b99f7f9${_scopeId}> Удалить у меня </div>`);
							else _push(`<!---->`);
							if (contextMenu.value.message?.is_mine) _push(`<div class="context-menu-item delete" data-v-7b99f7f9${_scopeId}> Удалить у всех </div>`);
							else _push(`<!---->`);
							_push(`</div>`);
						} else _push(`<!---->`);
						if (selectedMessages.value.length > 0) {
							_push(`<div class="selection-toolbar" data-v-7b99f7f9${_scopeId}><span data-v-7b99f7f9${_scopeId}>${ssrInterpolate(selectedMessages.value.length)} выбрано</span>`);
							if (canDeleteSelected.value) _push(`<button type="button" class="selection-delete-btn" data-v-7b99f7f9${_scopeId}> Удалить </button>`);
							else _push(`<!---->`);
							_push(`<button type="button" class="selection-clear-btn" data-v-7b99f7f9${_scopeId}> Отмена </button></div>`);
						} else _push(`<!---->`);
						if (showPriceModal.value) _push(`<div class="modal-overlay" style="${ssrRenderStyle({ "display": "flex" })}" data-v-7b99f7f9${_scopeId}><div class="price-modal" data-v-7b99f7f9${_scopeId}><div class="price-modal-header" data-v-7b99f7f9${_scopeId}><h3 data-v-7b99f7f9${_scopeId}>Предложить новую цену</h3><button type="button" class="modal-close" data-v-7b99f7f9${_scopeId}><img src="/images/close.svg" alt="Закрыть" data-v-7b99f7f9${_scopeId}></button></div><form data-v-7b99f7f9${_scopeId}><div class="price-modal-body" data-v-7b99f7f9${_scopeId}><label for="new-price" data-v-7b99f7f9${_scopeId}>Новая цена (₽):</label><input id="new-price"${ssrRenderAttr("value", priceForm.value.proposed_price)} type="number" min="0" max="9999999999" placeholder="Введите сумму" required data-v-7b99f7f9${_scopeId}></div><div class="price-modal-footer" data-v-7b99f7f9${_scopeId}><button type="button" class="cancel-btn" data-v-7b99f7f9${_scopeId}>Отмена</button><button type="submit" class="submit-btn" data-v-7b99f7f9${_scopeId}>Предложить</button></div></form></div></div>`);
						else _push(`<!---->`);
						if (priceChangeModal.value.show) _push(`<div class="modal-overlay" style="${ssrRenderStyle({ "display": "flex" })}" data-v-7b99f7f9${_scopeId}><div class="price-modal" data-v-7b99f7f9${_scopeId}><div class="price-modal-header" data-v-7b99f7f9${_scopeId}><h3 data-v-7b99f7f9${_scopeId}>Предложить новую цену</h3><button type="button" class="modal-close" data-v-7b99f7f9${_scopeId}><img src="/images/close.svg" alt="Закрыть" data-v-7b99f7f9${_scopeId}></button></div><form data-v-7b99f7f9${_scopeId}><div class="price-modal-body" data-v-7b99f7f9${_scopeId}><p class="price-modal-info" data-v-7b99f7f9${_scopeId}>Текущее предложение: ${ssrInterpolate(priceChangeModal.value.message?.proposed_price)} ₽</p><label for="new-price-change" data-v-7b99f7f9${_scopeId}>Новая цена (₽):</label><input id="new-price-change"${ssrRenderAttr("value", priceChangeModal.value.newPrice)} type="number" min="0" max="9999999999" placeholder="Введите сумму" required data-v-7b99f7f9${_scopeId}></div><div class="price-modal-footer" data-v-7b99f7f9${_scopeId}><button type="button" class="cancel-btn" data-v-7b99f7f9${_scopeId}>Отмена</button><button type="submit" class="submit-btn" data-v-7b99f7f9${_scopeId}>Предложить</button></div></form></div></div>`);
						else _push(`<!---->`);
						if (openDisputeModal.value) _push(`<div class="modal-overlay" style="${ssrRenderStyle({ "display": "flex" })}" data-v-7b99f7f9${_scopeId}><div class="price-modal" data-v-7b99f7f9${_scopeId}><div class="price-modal-header" data-v-7b99f7f9${_scopeId}><h3 data-v-7b99f7f9${_scopeId}>Открыть спор</h3><button type="button" class="modal-close" data-v-7b99f7f9${_scopeId}><img src="/images/close.svg" alt="Закрыть" data-v-7b99f7f9${_scopeId}></button></div><form data-v-7b99f7f9${_scopeId}><div class="price-modal-body" data-v-7b99f7f9${_scopeId}><label for="dispute-reason" data-v-7b99f7f9${_scopeId}>Причина спора:</label><textarea id="dispute-reason" rows="5" minlength="10" maxlength="5000" placeholder="Опишите причину спора (минимум 10 символов)" required data-v-7b99f7f9${_scopeId}>${ssrInterpolate(disputeForm.value.reason)}</textarea></div><div class="price-modal-footer" data-v-7b99f7f9${_scopeId}><button type="button" class="cancel-btn" data-v-7b99f7f9${_scopeId}>Отмена</button><button type="submit" class="submit-btn" data-v-7b99f7f9${_scopeId}>Отправить</button></div></form></div></div>`);
						else _push(`<!---->`);
						if (openCancelModal.value) _push(`<div class="modal-overlay" style="${ssrRenderStyle({ "display": "flex" })}" data-v-7b99f7f9${_scopeId}><div class="price-modal" data-v-7b99f7f9${_scopeId}><div class="price-modal-header" data-v-7b99f7f9${_scopeId}><h3 data-v-7b99f7f9${_scopeId}>Отмена сделки</h3><button type="button" class="modal-close" data-v-7b99f7f9${_scopeId}><img src="/images/close.svg" alt="Закрыть" data-v-7b99f7f9${_scopeId}></button></div><form data-v-7b99f7f9${_scopeId}><div class="price-modal-body" data-v-7b99f7f9${_scopeId}><label for="cancel-reason" data-v-7b99f7f9${_scopeId}>Причина отмены:</label><textarea id="cancel-reason" rows="5" minlength="5" maxlength="5000" placeholder="Опишите причину отмены (минимум 5 символов)" required data-v-7b99f7f9${_scopeId}>${ssrInterpolate(cancelForm.value.reason)}</textarea></div><div class="price-modal-footer" data-v-7b99f7f9${_scopeId}><button type="button" class="cancel-btn" data-v-7b99f7f9${_scopeId}>Назад</button><button type="submit" class="reject-btn" data-v-7b99f7f9${_scopeId}>Подтвердить отмену</button></div></form></div></div>`);
						else _push(`<!---->`);
					} else return [
...
var Chats_default = /* @__PURE__ */ _plugin_vue_export_helper_default(_sfc_main$15, [["__scopeId", "data-v-7b99f7f9"]]);
//#endregion
...
var _sfc_main$8 = /* @__PURE__ */ Object.assign({ layout: AppLayout_default }, {
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(_attrs)} data-v-0db76fad>`);
			_push(ssrRenderComponent(unref(Head), { title: __props.post.title }, null, _parent));
			_push(`<div class="block" data-v-0db76fad><div class="image-container" data-v-0db76fad>`);
			if (__props.post.image_url) _push(`<img${ssrRenderAttr("src", __props.post.image_url)}${ssrRenderAttr("alt", __props.post.title)} class="post-image" data-v-0db76fad>`);
			else _push(`<div class="no-image" data-v-0db76fad><span data-v-0db76fad>Изображение отсутствует</span></div>`);
			_push(`</div><div class="content-wrapper" data-v-0db76fad>`);
			if (__props.post.is_hidden) _push(`<div class="hidden-warning" data-v-0db76fad> ⚠️ Этот пост скрыт администрацией и виден только вам </div>`);
			else _push(`<!---->`);
			_push(`<div class="desc" data-v-0db76fad><div class="header-actions" data-v-0db76fad>`);
			if (__props.post.is_vacancy) _push(`<h2 class="title" data-v-0db76fad>Вакансия: ${ssrInterpolate(__props.post.vacancy.position)}</h2>`);
			else _push(`<h2 class="title" data-v-0db76fad>${ssrInterpolate(__props.post.title)}</h2>`);
			if (canEdit.value || canReport.value) {
				_push(`<div class="menu-container" data-v-0db76fad><button class="menu-btn" type="button" data-v-0db76fad><img src="/images/dots.svg" alt="меню" data-v-0db76fad></button>`);
				if (menuOpen.value) {
					_push(`<div class="post-dropdown-menu" data-v-0db76fad><button class="menu-item" data-v-0db76fad><img src="/images/share.svg" alt="" data-v-0db76fad>Поделиться </button>`);
					if (__props.post.edit_url) _push(ssrRenderComponent(unref(Link), {
						href: __props.post.edit_url,
						class: "menu-item"
					}, {
						default: withCtx((_, _push, _parent, _scopeId) => {
							if (_push) _push(`<img src="/images/edit.svg" alt="" data-v-0db76fad${_scopeId}>Редактировать `);
							else return [createVNode("img", {
								src: "/images/edit.svg",
								alt: ""
					...
					if (__props.post.delete_url) _push(`<button class="menu-item delete" data-v-0db76fad><img src="/images/trash.svg" alt="" data-v-0db76fad> Удалить </button>`);
					else _push(`<!---->`);
					if (canReport.value) _push(`<button class="menu-item" data-v-0db76fad><img src="/images/flag.svg" alt="" data-v-0db76fad>Пожаловаться </button>`);
					else _push(`<!---->`);
					_push(`</div>`);
				} else _push(`<!---->`);
				_push(`</div>`);
			...
			_push(ssrRenderComponent(unref(Link), {
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						if (__props.post.user.avatar_url) _push(`<img${ssrRenderAttr("src", __props.post.user.avatar_url)} class="author-avatar"${ssrRenderAttr("alt", __props.post.user.name)} data-v-0db76fad${_scopeId}>`);
						else _push(`<img src="/images/User-avatar.png" class="author-avatar"${ssrRenderAttr("alt", __props.post.user.name)} data-v-0db76fad${_scopeId}>`);
			...
			_push(`<div class="post-actions" data-v-0db76fad><button type="button" class="${ssrRenderClass([{ liked: __props.post.is_liked }, "like-btn"])}" data-v-0db76fad>${ssrInterpolate(__props.post.is_liked ? "❤️" : "🤍")} ${ssrInterpolate(__props.post.likes_count)}</button></div>`);
			if (__props.post.is_vacancy) {
				_push(`<!--[--><div class="vacancy-info" data-v-0db76fad>`);
				if (__props.post.vacancy.budget_min || __props.post.vacancy.budget_max) _push(`<div class="vacancy-budget" data-v-0db76fad><span class="label" data-v-0db76fad>Бюджет:</span><span class="value" data-v-0db76fad>${ssrInterpolate(__props.post.vacancy.budget_min ? __props.post.vacancy.budget_min + " ₽" : "")} ${ssrInterpolate(__props.post.vacancy.budget_min && __props.post.vacancy.budget_max ? " - " : "")} ${ssrInterpolate(__props.post.vacancy.budget_max ? __props.post.vacancy.budget_max + " ₽" : "")}</span></div>`);
				else _push(`<!---->`);
				if (__props.post.vacancy.deadline) _push(`<div class="vacancy-deadline" data-v-0db76fad><span class="label" data-v-0db76fad>Срок:</span><span class="value" data-v-0db76fad>${ssrInterpolate(formatDeadline(__props.post.vacancy.deadline))}</span></div>`);
				else _push(`<!---->`);
				if (__props.post.vacancy.skills && __props.post.vacancy.skills.length > 0) {
					_push(`<div class="vacancy-skills" data-v-0db76fad><span class="label" data-v-0db76fad>Требуемые навыки:</span><div class="skills-list" data-v-0db76fad><!--[-->`);
					ssrRenderList(__props.post.vacancy.skills, (skill) => {
						_push(`<span class="${ssrRenderClass([unref(getSkillClass)(skill.name), "skill-tag"])}" data-v-0db76fad>${ssrInterpolate(skill.name)} <span class="skill-level" data-v-0db76fad>★ ${ssrInterpolate(skill.level)}</span></span>`);
					});
					_push(`<!--]--></div>`);
					if (__props.post.vacancy.requirements) _push(`<div class="vacancy-requirements" data-v-0db76fad><span class="label" data-v-0db76fad>Требования:</span><p class="requirements-text" data-v-0db76fad>${ssrInterpolate(__props.post.vacancy.requirements)}</p></div>`);
					else _push(`<!---->`);
					_push(`</div>`);
				} else _push(`<!---->`);
				_push(`</div>`);
				if (__props.post.vacancy && __props.post.vacancy.status === "open") {
					_push(`<div class="vacancy-actions" data-v-0db76fad>`);
					if (__props.post.vacancy.applications_count > 0) _push(`<div class="applications-count" data-v-0db76fad>${ssrInterpolate(__props.post.vacancy.applications_count)} ${ssrInterpolate(getApplicationsWord(__props.post.vacancy.applications_count))} на эту вакансию </div>`);
					else _push(`<!---->`);
					if (!isAuthor.value && __props.post.respond_url) _push(`<button class="respond-btn"${ssrIncludeBooleanAttr(__props.post.has_application) ? " disabled" : ""} data-v-0db76fad>${ssrInterpolate(__props.post.has_application ? "Вы уже откликнулись" : "Откликнуться")}</button>`);
					else _push(`<!---->`);
					_push(`</div>`);
				} else _push(`<!---->`);
				_push(`<!--]-->`);
			} else _push(`<p class="description" data-v-0db76fad>${ssrInterpolate(__props.post.description)}</p>`);
			_push(`<div class="meta" data-v-0db76fad><small data-v-0db76fad>${ssrInterpolate(formattedDate.value)}</small></div></div></div></div><div class="comments-section-wrapper" data-v-0db76fad>`);
			if (__props.post.is_vacancy && canEdit.value) _push(`<div class="tabs" data-v-0db76fad><button class="${ssrRenderClass([{ active: activeTab.value === "applications" }, "tab"])}" data-v-0db76fad> Список откликнувшихся (${ssrInterpolate(__props.post.vacancy.applications?.length || 0)}) </button><button class="${ssrRenderClass([{ active: activeTab.value === "comments" }, "tab"])}" data-v-0db76fad> Комментарии (${ssrInterpolate(__props.post.comments.length)}) </button></div>`);
			else _push(`<!---->`);
			_push(`<div class="comments-section" data-v-0db76fad>`);
			if (!__props.post.is_vacancy || !canEdit.value || activeTab.value === "comments") {
				_push(`<!--[-->`);
				if (_ctx.$page.props.auth.user) {
					_push(`<div class="comment-form" data-v-0db76fad><form data-v-0db76fad>`);
					if (commentErrors.value.text) _push(`<div class="error" data-v-0db76fad>${ssrInterpolate(commentErrors.value.text)}</div>`);
					else _push(`<!---->`);
					_push(`<textarea required placeholder="Напишите комментарий"${ssrIncludeBooleanAttr(unref(commentForm).processing) ? " disabled" : ""} data-v-0db76fad>${ssrInterpolate(unref(commentForm).text)}</textarea><button type="submit"${ssrIncludeBooleanAttr(unref(commentForm).processing) ? " disabled" : ""} data-v-0db76fad>${ssrInterpolate(unref(commentForm).processing ? "Отправка..." : "Добавить комментарий")}</button></form></div>`);
				} else {
					_push(`<div class="login-prompt" data-v-0db76fad><p data-v-0db76fad>Чтобы оставить комментарий, `);
					_push(ssrRenderComponent(unref(Link), { href: "/login" }, {
						default: withCtx((_, _push, _parent, _scopeId) => {
				...
				_push(`<h3 data-v-0db76fad>Комментарии (${ssrInterpolate(__props.post.comments.length)})</h3>`);
				if (__props.post.comments.length === 0) _push(`<div class="no-comments" data-v-0db76fad><p data-v-0db76fad>Комментариев пока нет. Будьте первым!</p></div>`);
				else {
					_push(`<div class="comments" data-v-0db76fad><!--[-->`);
					ssrRenderList(__props.post.comments, (comment) => {
						_push(`<div class="comment" data-v-0db76fad><div class="comment-header" data-v-0db76fad>`);
						_push(ssrRenderComponent(unref(Link), {
							...
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) {
									if (comment.user.avatar_url) _push(`<img${ssrRenderAttr("src", comment.user.avatar_url)} class="comment-avatar"${ssrRenderAttr("alt", comment.user.name)} data-v-0db76fad${_scopeId}>`);
									else _push(`<img src="/images/User-avatar.png" class="comment-avatar"${ssrRenderAttr("alt", comment.user.name)} data-v-0db76fad${_scopeId}>`);
									_push(`<span class="comment-author-name" data-v-0db76fad${_scopeId}>${ssrInterpolate(comment.user.name)}</span>`);
						...
						_push(`<small class="comment-date" data-v-0db76fad>${ssrInterpolate(formatDate(comment.created_at))}</small></div><div class="comment-body" data-v-0db76fad><p data-v-0db76fad>${ssrInterpolate(comment.text)}</p></div></div>`);
					});
					_push(`<!--]--></div>`);
				}
				_push(`<!--]-->`);
			} else if (activeTab.value === "applications") {
				_push(`<!--[--><h3 data-v-0db76fad>Список откликнувшихся (${ssrInterpolate(__props.post.vacancy.applications?.length || 0)})</h3>`);
				if (!__props.post.vacancy.applications || __props.post.vacancy.applications.length === 0) _push(`<div class="no-comments" data-v-0db76fad><p data-v-0db76fad>На эту вакансию пока никто не откликнулся.</p></div>`);
				else {
					_push(`<div class="comments" data-v-0db76fad><!--[-->`);
					ssrRenderList(__props.post.vacancy.applications, (application) => {
						...
						_push(ssrRenderComponent(unref(Link), {
							...
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) {
									if (application.user.avatar_url) _push(`<img${ssrRenderAttr("src", application.user.avatar_url)} class="comment-avatar"${ssrRenderAttr("alt", application.user.name)} data-v-0db76fad${_scopeId}>`);
									else _push(`<img src="/images/User-avatar.png" class="comment-avatar"${ssrRenderAttr("alt", application.user.name)} data-v-0db76fad${_scopeId}>`);
									_push(`<span class="comment-author-name" data-v-0db76fad${_scopeId}>${ssrInterpolate(application.user.name)}</span>`);
						...
						_push(`<div class="comment-header-right" data-v-0db76fad><small class="comment-date" data-v-0db76fad>${ssrInterpolate(formatDate(application.created_at))}</small></div></div><div class="comment-body" data-v-0db76fad><div data-v-0db76fad>`);
						if (application.cover_letter) _push(`<p data-v-0db76fad>${ssrInterpolate(application.cover_letter)}</p>`);
						else _push(`<!---->`);
						if (application.proposed_price) _push(`<p class="proposed-price" data-v-0db76fad>Предложенная цена: ${ssrInterpolate(application.proposed_price)} ₽</p>`);
						else _push(`<!---->`);
						_push(`</div>`);
			...
			if (showRespondModal.value) {
				_push(`<div class="modal-overlay" data-v-0db76fad><div class="modal-content" data-v-0db76fad><button class="modal-close" data-v-0db76fad>×</button><h2 data-v-0db76fad>Отклик на вакансию</h2><form data-v-0db76fad><div class="form-group" data-v-0db76fad><label for="cover_letter" data-v-0db76fad>Сопроводительное письмо *</label><textarea id="cover_letter" required placeholder="Расскажите о себе и почему вы подходите на эту вакансию..." rows="6" data-v-0db76fad>${ssrInterpolate(unref(respondForm).cover_letter)}</textarea>`);
				if (unref(respondForm).errors.cover_letter) _push(`<div class="error" data-v-0db76fad>${ssrInterpolate(unref(respondForm).errors.cover_letter)}</div>`);
				else _push(`<!---->`);
				_push(`</div><div class="form-group" data-v-0db76fad><label for="proposed_price" data-v-0db76fad>Предложенная цена (₽)</label><input type="number" id="proposed_price"${ssrRenderAttr("value", unref(respondForm).proposed_price)} min="1" max="9999999999" placeholder="Ваша цена" data-v-0db76fad>`);
				if (unref(respondForm).errors.proposed_price) _push(`<div class="error" data-v-0db76fad>${ssrInterpolate(unref(respondForm).errors.proposed_price)}</div>`);
				else _push(`<!---->`);
				_push(`</div><button type="submit" class="submit-btn"${ssrIncludeBooleanAttr(unref(respondForm).processing) ? " disabled" : ""} data-v-0db76fad>${ssrInterpolate(unref(respondForm).processing ? "Отправка..." : "Отправить отклик")}</button></form></div></div>`);
			} else _push(`<!---->`);
			if (showReportModal.value) {
				_push(`<div class="modal-overlay" data-v-0db76fad><div class="modal-content" data-v-0db76fad><button class="modal-close" data-v-0db76fad>×</button><h2 data-v-0db76fad>Пожаловаться на пост</h2><form data-v-0db76fad><div class="form-group" data-v-0db76fad><label for="report_reason" data-v-0db76fad>Причина жалобы *</label><textarea id="report_reason" required placeholder="Опишите причину жалобы..." rows="4" data-v-0db76fad>${ssrInterpolate(unref(reportForm).reason)}</textarea>`);
				if (reportErrors.value.reason) _push(`<div class="error" data-v-0db76fad>${ssrInterpolate(reportErrors.value.reason)}</div>`);
				else _push(`<!---->`);
				_push(`</div><button type="submit" class="submit-btn"${ssrIncludeBooleanAttr(unref(reportForm).processing) ? " disabled" : ""} data-v-0db76fad>${ssrInterpolate(unref(reportForm).processing ? "Отправка..." : "Отправить жалобу")}</button></form></div></div>`);
			} else _push(`<!---->`);
			if (showShareModal.value) {
				_push(`<div class="modal-overlay" data-v-0db76fad><div class="modal-content" data-v-0db76fad><button class="modal-close" data-v-0db76fad>×</button><h2 data-v-0db76fad>Поделиться постом</h2><form data-v-0db76fad><div class="form-group" data-v-0db76fad><label data-v-0db76fad>Выберите чаты для отправки *</label>`);
				if (sharedChats.value.length === 0) _push(`<div class="no-chats" data-v-0db76fad><p data-v-0db76fad>У вас нет чатов для отправки поста.</p></div>`);
				else {
					_push(`<div class="chat-list" data-v-0db76fad><!--[-->`);
					ssrRenderList(sharedChats.value, (chat) => {
						_push(`<div class="chat-item" data-v-0db76fad><label class="chat-label" data-v-0db76fad><input type="checkbox"${ssrRenderAttr("value", chat.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(shareForm).users) ? ssrLooseContain(unref(shareForm).users, chat.id) : unref(shareForm).users) ? " checked" : ""} data-v-0db76fad><span class="chat-info" data-v-0db76fad>`);
						if (chat.other_user && chat.other_user.avatar_url) _push(`<img${ssrRenderAttr("src", chat.other_user.avatar_url)} class="chat-avatar"${ssrRenderAttr("alt", chat.other_user.name)} data-v-0db76fad>`);
						else _push(`<img src="/images/User-avatar.png" class="chat-avatar"${ssrRenderAttr("alt", chat.other_user ? chat.other_user.name : "Неизвестный пользователь")} data-v-0db76fad>`);
						_push(`<span class="chat-name" data-v-0db76fad>${ssrInterpolate(chat.other_user ? chat.other_user.name : "Неизвестный пользователь")}</span></span></label></div>`);
					});
					_push(`<!--]--></div>`);
				}
				if (shareErrors.value.users) _push(`<div class="error" data-v-0db76fad>${ssrInterpolate(shareErrors.value.users)}</div>`);
				else _push(`<!---->`);
				_push(`</div><div class="form-group" data-v-0db76fad><label for="share_message" data-v-0db76fad>Текст сообщения</label><textarea id="share_message" placeholder="Напишите сообщение к посту..." rows="4" data-v-0db76fad>${ssrInterpolate(unref(shareForm).message)}</textarea></div><button type="submit" class="submit-btn"${ssrIncludeBooleanAttr(unref(shareForm).processing) ? " disabled" : ""} data-v-0db76fad>${ssrInterpolate(unref(shareForm).processing ? "Отправка..." : "Отправить")}</button></form></div></div>`);
			} else _push(`<!---->`);
			_push(`</div>`);
		};
	}
...
var Show_default$1 = /* @__PURE__ */ _plugin_vue_export_helper_default(_sfc_main$8, [["__scopeId", "data-v-0db76fad"]]);
//#endregion
			...
			_push(ssrRenderComponent(AppLayout_default, _attrs, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						_push(`<div class="Profileblock" data-v-5818609c${_scopeId}><div class="container" data-v-5818609c${_scopeId}><div class="avatar" data-v-5818609c${_scopeId}><div class="profile-header" data-v-5818609c${_scopeId}>`);
						if (__props.user.avatar_url) _push(`<img${ssrRenderAttr("src", __props.user.avatar_url)}${ssrRenderAttr("alt", "Аватарка " + __props.user.name)} data-v-5818609c${_scopeId}>`);
						else _push(`<img src="/images/User-avatar.png"${ssrRenderAttr("alt", "Аватарка " + __props.user.name)} data-v-5818609c${_scopeId}>`);
						_push(`</div><div class="desc" data-v-5818609c${_scopeId}><h1 data-v-5818609c${_scopeId}>${ssrInterpolate(__props.user.name)}</h1>`);
						if (__props.user.is_verified === "verified") _push(`<img src="/images/verified.svg" alt="Аккаунт верифицирован" data-v-5818609c${_scopeId}>`);
						else _push(`<!---->`);
						_push(`</div>`);
						if (__props.user.rating) {
							_push(`<div class="rating-display" data-v-5818609c${_scopeId}>`);
							_push(ssrRenderComponent(unref(Link), {
								...
								default: withCtx((_, _push, _parent, _scopeId) => {
									if (_push) {
										_push(`<div class="stars" data-v-5818609c${_scopeId}><!--[-->`);
										ssrRenderList(5, (i) => {
											_push(`<!--[-->`);
											if (i <= Math.floor(__props.user.rating)) _push(`<img src="/images/star.svg" alt="star" class="star-icon" data-v-5818609c${_scopeId}>`);
											else if (i - 1 < __props.user.rating && __props.user.rating % 1 >= .5) _push(`<img src="/images/star.svg" alt="half-star" class="star-icon half" data-v-5818609c${_scopeId}>`);
											else _push(`<img src="/images/star-empty.svg" alt="star-empty" class="star-icon empty" data-v-5818609c${_scopeId}>`);
											_push(`<!--]-->`);
										});
										_push(`<!--]--></div><span class="rating-value" data-v-5818609c${_scopeId}>${ssrInterpolate(__props.user.rating)}</span>`);
						...
						if (__props.user.created_at) _push(`<p class="created-at" data-v-5818609c${_scopeId}>Аккаунт создан ${ssrInterpolate(__props.user.created_at)}</p>`);
						else _push(`<!---->`);
						_push(`</div></div><div class="aboutme" data-v-5818609c${_scopeId}><div class="aboutme-header" data-v-5818609c${_scopeId}><h1 data-v-5818609c${_scopeId}>Обо мне</h1>`);
						_push(ssrRenderComponent(unref(Link), { href: "/profile/" + __props.user.id + "/following" }, {
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) _push(` Подписки<br data-v-5818609c${_scopeId}> ${ssrInterpolate(__props.user.following_count)}`);
								else return [
						...
						_push(ssrRenderComponent(unref(Link), { href: "/profile/" + __props.user.id + "/followers" }, {
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) _push(` Подписчики<br data-v-5818609c${_scopeId}> ${ssrInterpolate(__props.user.followers_count)}`);
								else return [
						...
						if (isOwnProfile.value) _push(ssrRenderComponent(unref(Link), {
							...
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) _push(`<img src="/images/settings.svg" alt="Редактировать профиль" data-v-5818609c${_scopeId}>`);
								else return [createVNode("img", {
									src: "/images/settings.svg",
						...
						if (isOwnProfile.value) {
							_push(`<div data-v-5818609c${_scopeId}>`);
							if (!__props.user.aboutme) _push(`<div class="aboutme-form" data-v-5818609c${_scopeId}><p data-v-5818609c${_scopeId}>Расскажите о себе</p><form data-v-5818609c${_scopeId}><textarea draggable="false" placeholder="Напишите что-нибудь о себе..." data-v-5818609c${_scopeId}>${ssrInterpolate(unref(aboutMeForm).aboutme)}</textarea><button type="submit"${ssrIncludeBooleanAttr(unref(aboutMeForm).processing) ? " disabled" : ""} data-v-5818609c${_scopeId}>Сохранить</button></form></div>`);
							else _push(`<div class="${ssrRenderClass([{ expanded: isExpanded.value }, "aboutme-content"])}" style="${ssrRenderStyle({ maxHeight: expandedHeight.value })}" data-v-5818609c${_scopeId}><p data-v-5818609c${_scopeId}>${ssrInterpolate(__props.user.aboutme)}</p></div>`);
							_push(`</div>`);
						} else _push(`<div class="aboutme-body" data-v-5818609c${_scopeId}><div class="${ssrRenderClass([{ expanded: isExpanded.value }, "aboutme-content"])}" style="${ssrRenderStyle({ maxHeight: expandedHeight.value })}" data-v-5818609c${_scopeId}><p class="truncated-text" data-v-5818609c${_scopeId}>${ssrInterpolate(__props.user.aboutme || "Пользователь пока не добавил информацию о себе.")}</p></div></div>`);
						if (showExpandButton.value) _push(`<div class="expand" data-v-5818609c${_scopeId}>${ssrInterpolate(isExpanded.value ? "Свернуть" : "Развернуть")}</div>`);
						else _push(`<!---->`);
						if (__props.user.skills && __props.user.skills.length > 0) {
							_push(`<div class="user-skills" data-v-5818609c${_scopeId}><h3 data-v-5818609c${_scopeId}>Навыки</h3><div class="skills-list" data-v-5818609c${_scopeId}><!--[-->`);
							ssrRenderList(__props.user.skills, (skill) => {
								_push(`<div class="${ssrRenderClass([unref(getSkillClass)(skill.name), "skill-tag"])}" data-v-5818609c${_scopeId}><span class="skill-name" data-v-5818609c${_scopeId}>${ssrInterpolate(skill.name)}</span><span class="skill-level" data-v-5818609c${_scopeId}>★ ${ssrInterpolate(skill.level)}</span></div>`);
							});
							_push(`<!--]--></div></div>`);
						} else _push(`<!---->`);
						if (!isOwnProfile.value && auth.value.user) {
							_push(`<div class="profile-actions" data-v-5818609c${_scopeId}><form data-v-5818609c${_scopeId}><button type="submit" data-v-5818609c${_scopeId}>${ssrInterpolate(__props.user.is_subscribed ? "Отписаться" : "Подписаться")}</button></form>`);
							_push(ssrRenderComponent(unref(Link), { href: "/chats/start/" + __props.user.id }, {
								default: withCtx((_, _push, _parent, _scopeId) => {
									if (_push) _push(`<button data-v-5818609c${_scopeId}>Написать сообщение</button>`);
						...
						_push(`</div></div><h2 data-v-5818609c${_scopeId}>Посты ${ssrInterpolate(__props.user.name)}</h2>`);
						if (__props.posts.length > 0 || isOwnProfile.value) {
							_push(`<div class="posts" data-v-5818609c${_scopeId}>`);
							if (isOwnProfile.value) {
								_push(`<div class="post add-post-block" data-v-5818609c${_scopeId}>`);
								_push(ssrRenderComponent(unref(Link), {
									href: "/posts/create",
									class: "add-post-link"
								}, {
									default: withCtx((_, _push, _parent, _scopeId) => {
										if (_push) _push(`<p data-v-5818609c${_scopeId}>Добавить новый пост</p><button class="add-post-button" data-v-5818609c${_scopeId}>+</button>`);
							...
							_push(`<!--]--></div>`);
						} else _push(`<div class="noposts" data-v-5818609c${_scopeId}><h1 data-v-5818609c${_scopeId}>у ${ssrInterpolate(__props.user.name)} пока что нет постов...</h1></div>`);
						if (isOwnProfile.value && __props.closedVacancies && __props.closedVacancies.length > 0) {
							_push(`<div class="closed-vacancies" data-v-5818609c${_scopeId}><h2 data-v-5818609c${_scopeId}>Завершённые вакансии</h2><div class="posts" data-v-5818609c${_scopeId}><!--[-->`);
...
var Show_default = /* @__PURE__ */ _plugin_vue_export_helper_default(_sfc_main$2, [["__scopeId", "data-v-5818609c"]]);
//#endregion
